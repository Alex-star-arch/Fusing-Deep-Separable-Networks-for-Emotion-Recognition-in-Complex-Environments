...
class Emotion_Rec:
    def __init__(self, model_path=None):
        ...
        if model_path == None: # 若未指定路径，则使用默认模型
            emotion_model_path = 'models/_mini_XCEPTION.102-0.66.hdf5' # 默认模型选定
        else:
    ...
    def run(self, frame_in, canvas, label_face, label_result):
        ...
        if len(faces) > 0:
            ...
            # 从灰度图中提取感兴趣区域（ROI），将其大小转换为64*64 pixels，并为通过CNN的分类器准备ROI #fix:input data need to be 48
            roi = gray[fY:fY + fH, fX:fX + fW]
            roi = cv2.resize(roi, (64, 64))#(48, 48)
        ...
        # 调整画面大小与界面相适应
        frameClone = cv2.resize(frameClone,(560,370))
